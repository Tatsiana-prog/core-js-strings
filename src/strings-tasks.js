...
function extractNameFromTemplate(value) {
  const regex = /Hello,\s*([^ ]+)\s+([^!]+)/;
  const match = value.match(regex);
  if (match) {
    return `${match[1]} ${match[2]}`; // Возвращаем полное имя
  }
  return null; // Если шаблон не совпадает, возвращаем null
}
/**
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, (char) => {
    const startCode = char <= 'Z' ? 65 : 97; // Код ASCII для 'A' или 'a'
